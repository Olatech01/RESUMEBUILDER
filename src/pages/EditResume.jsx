...
const EditResume = () => {
    ...
    const fetchResumeDetailsById = async () => {
        try {
            const response = await axiosInstance.get(API_PATH.RESUME.GET_BY_ID(id))
    ...
    const dataURLtoFile = (dataurl, filename) => {
        ...
        try {
            ...
            // Fallback for older browsers: return a Blob with name attached
    ...
    const uploadResumeImages = async () => {
        try {
            ...
            const uploadResponse = await axiosInstance.put(
                API_PATH.RESUME.UPLOAD_IMAGES(id),
                formData,
                {
    ...
    const updateResumeDetails = async (thumbnailLink) => {
        try {
            ...
            await axiosInstance.put(API_PATH.RESUME.UPDATE(id), {
                ...resumeData,
    ...
    const handleDeleteResume = async () => {
        try {
            setIsLoading(true)
            await axiosInstance.delete(API_PATH.RESUME.DELETE(id))
                        ...
                        <div className='p-4 sm:p-6'>
                            ...
                            <div className='flex flex-wrap items-center justify-end gap-3'>
                                ...
                                <button className={buttonStyles.next} onClick={validateAndNext} disabled={isLoading}>
                                    ...
                                    {currentPage === "additionalInfor" && <ArrowLeft size={16} className='rotate-180' />}
                                </button>
                            </div>
                        </div>
                    </div>
